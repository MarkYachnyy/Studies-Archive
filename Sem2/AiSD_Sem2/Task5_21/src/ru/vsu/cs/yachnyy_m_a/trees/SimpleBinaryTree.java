...
public class SimpleBinaryTree<T> implements BinaryTree<T> {
    ...
    public T get(String path){
        return get(getRoot(), path);
    }

    public TreeNode<T> getNode(String path){
        return getNode(getRoot(), path);
    }

    private TreeNode<T> getNode(TreeNode<T> node, String path){
        if(path.equals("")){
            return node;
        } else {
            return getNode(path.charAt(0) == 'L' ? node.getLeft() : node.getRight(), path.substring(1));
        }
    }

    private T get(TreeNode<T> node, String path){
        if(path.equals("")){
            return node.getValue();
        } else {
            return get(path.charAt(0) == 'L' ? node.getLeft() : node.getRight(), path.substring(1));
        }
    }
}
